...
def compute_fisher_params(M0, B0, M1, B1):
    ...
    Возвращает W, wN.
    """
    # Обобщённая матрица ковариации: (B0 + B1)/2
    B_avg = 0.5 * (B0 + B1)
    B_avg_inv = inv(B_avg)
    W = B_avg_inv @ (M1 - M0)
    # Вычисляем дисперсии d(X) в классах
    sigma0_2 = W.T @ B0 @ W
    sigma1_2 = W.T @ B1 @ W
...
def compute_mse_params(M0, B0, M1, B1, P0=0.5, P1=0.5):
    ...
    """
    # Математические ожидания и корреляции для каждого класса
    E0 = np.concatenate([M0, [1.0]])
    E1 = np.concatenate([M1, [1.0]])
    # E[Z Z^T] = [[B + M M^T, M], [M^T, 1]]
    R0 = np.zeros((3, 3))
    ...
    R1[2, :2] = M1
    R1[2, 2] = 1.0
    # Смешанные моменты E[Z t]
    M_z_t = P0 * (-1.0) * E0 + P1 * (+1.0) * E1
    # Матрица E[Z Z^T]
    R_zz = P0 * R0 + P1 * R1
    # Решение системы R_zz * W_ext = M_z_t
    W_ext = inv(R_zz) @ M_z_t
    W = W_ext[:2]          # коэффициенты при признаках
    wN = W_ext[2]          # порог
    return W, wN
...
def plot_decision_boundary(X0, X1, classifier_func, title, filename, xlim=None, ylim=None, extra_boundaries=None):
    """
    Вспомогательная функция для визуализации разделяющей границы.
    extra_boundaries: список кортежей (W, wN, label, color, linestyle) для дополнительных линий.
    """
    ...
    Z = np.array([[classifier_func(np.array([x, y])) for x in xx_row] for y in yy])
...
def task2_mse():
    ...
    print(f"  Фишер:     p0={p0_f:.4f}, p1={p1_f:.4f}")
    print(f"  Байес:     p0={p0_b:.4f}, p1={p1_b:.4f}")
    # Визуализация (линии Фишера и MSE)
...
def robbins_monro_akp(X0, X1, alphas, W_init=None):
    ...
    """
    # Формируем расширенные векторы x_ext = (x0, x1, 1)
    # Для класса 0 используем знак минус? В алгоритме АКП (п.3.1.7) не требуется менять знак,
    # просто используем r(x) = +1 для класса 1 и r(x) = -1 для класса 0.
    # Для простоты создадим один массив всех расширенных векторов и соответствующих r.
    N0 = len(X0)
    N1 = len(X1)
    X_ext = np.vstack([np.hstack([X0, np.ones((N0, 1))]),
                       np.hstack([X1, np.ones((N1, 1))])])
    r = np.concatenate([-np.ones(N0), np.ones(N1)])
    # Перемешаем данные (для лучшей сходимости)
    ...
    if W_init is None:
        ...
        W = W_init.copy()
    history_W = [W.copy()]
    errors = []  # доля ошибок после каждой эпохи (проход по всем данным)
    # Проходим по данным многократно (эпохи)
    ...
    for k, alpha in enumerate(alphas):
        ...
        # Каждую эпоху (после полного прохода) вычисляем ошибку
        if (k+1) % len(r) == 0:
            epoch += 1
            # Оцениваем текущий классификатор на всей выборке
            pred = (X_ext @ W) > 0
            err = np.mean(pred != (r == 1))
            errors.append(err)
            if epoch >= 200:  # ограничим для скорости
                break
...
def task3_robbins_monro():
    ...
    # Параметры последовательности α_k = 1/k^β
    beta = 0.6
    max_iter = 5000
    alphas = np.array([1.0 / (i+1)**beta for i in range(max_iter)])
    
    # Функция для исследования и построения графиков
    def investigate(data_pair, title_tag):
        X0, X1 = data_pair
        # Начальные условия: нулевой вектор и случайный
        init_zero = np.zeros(3)
        init_rand = np.random.randn(3) * 0.1
        # Запускаем АКП и НСКО
        ...
        # Визуализация итоговой границы (для НСКО с нулевым началом)
